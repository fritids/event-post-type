 ...
 */
jQuery(function($){if($("#eventsCalendar").length){var events={};var hoverClass="ec-hover";var today=getDateFromURL();var displayedMonth=today.getMonth();var displayedYear=today.getFullYear();var $c=$("#eventsCalendar");$c.empty();$c.append($('<div id="ec-info"></div>'),$('<div class="ec-container"></div>').append($("<h4></h4>"),$('<div class="ec-nav-prev ec-nav"></div>').append($('<a class="ec-nav-prev-year" href="#" title="Previous Year">&lt;&lt;</a>').bind("click",function(){return displayNewMonth(this,0,-1);}),$('<a class="ec-nav-prev-month" href="#" title="Previous Month">&lt;</a>').bind("click",function(){return displayNewMonth(this,-1,0);})),$('<div class="ec-nav-next ec-nav"></div>').append($('<a class="ec-nav-next-year" href="#" title="Next Year">&gt;&gt;</a>').bind("click",function(){return displayNewMonth(this,0,1);}),$('<a class="ec-nav-next-month" href="#" title="Next Month">&gt;</a>').bind("click",function(){return displayNewMonth(this,1,0);})),$("<div></div>").attr("id","ec-calendar")));$(".ec-nav").show();getEvents(displayedMonth,displayedYear,function(){$("h4",$c).html(Date.monthNames[displayedMonth]+" "+displayedYear);renderCalendar(displayedMonth,displayedYear);});}function dc(a){return document.createElement(a);}function getDateFromURL(){var regex=new RegExp("([0-9]{4})/([0-9]{2})");var results=regex.exec(window.location.href);if(results===null){return(new Date()).zeroTime();}else{return(new Date(results[1],(parseInt(results[2])-1),1)).zeroTime();}}function addEvents(td,currentDate,month,year){var ev=events["for-"+month+"-"+year]||false;if(ev){var e4d=[];for(i=0;i<ev.length;i++){var ds=currentDate.getTime();var de=currentDate.getTime()+86399999;if(ev[i].start_jstimestamp>=ds){if(ev[i].start_jstimestamp<de){e4d.push(i);}}else{if(ev[i].end_jstimestamp>ds){e4d.push(i);}}}if(e4d.length){var eventText="";for(j=0;j<e4d.length;j++){eventText+="<h3>"+ev[e4d[j]].title+"</h3><p>"+ev[e4d[j]].dateStr+"</p>";}var dayHREF=eventsJSON.substr(0,eventsJSON.length-5)+"/"+currentDate.getFullYear()+"/"+getTwoDigits((currentDate.getMonth()+1))+"/"+getTwoDigits(currentDate.getDate());td.addClass("event").tooltip({title:function(){return $(eventText);}}).click(function(){window.location.href=dayHREF;});}}}function getEvents(month,year,callback){if(!events["for-"+month+"-"+year]){$.getJSON(eventsJSON+"/"+year+"/"+getTwoDigits((month+1)),function(data){events["for-"+month+"-"+year]=data;callback();});}else{callback();}}function getTwoDigits(num){var num=parseInt(num);if(num<10){return"0"+num;}else{return""+num;}}function renderCalendar(month,year){var headRow=$(dc("tr"));for(var i=Date.firstDayOfWeek;i<Date.firstDayOfWeek+7;i++){var weekday=i%7;var day=Date.dayNames[weekday];headRow.append(jQuery(dc("th")).attr({scope:"col",abbr:day,title:day,"class":(weekday==0||weekday==6?"weekend":"weekday")}).html(day.substr(0,1)));}var calendarTable=$(dc("table")).attr({cellspacing:2,"class":"eventsCalendar"}).append($(dc("thead")).append(headRow));var tbody=$(dc("tbody"));var today=(new Date()).zeroTime();var currentDate=new Date(year,month,1);var firstDayOffset=Date.firstDayOfWeek-currentDate.getDay()+1;if(firstDayOffset>1){firstDayOffset-=7;}var weeksToDraw=Math.ceil(((-1*firstDayOffset+1)+currentDate.getDaysInMonth())/7);currentDate.addDays(firstDayOffset-1);var doHover=function(){$(this).addClass(hoverClass);};var unHover=function(){$(this).removeClass(hoverClass);};var w=0;while(w++<weeksToDraw){var r=jQuery(dc("tr"));for(var i=0;i<7;i++){var thisMonth=currentDate.getMonth()==month;var d=$(dc("td")).text(currentDate.getDate()+"").attr("class",(thisMonth?"current-month ":"other-month ")+(currentDate.isWeekend()?"weekend ":"weekday ")+(thisMonth&&currentDate.getTime()==today.getTime()?"today ":"")).hover(doHover,unHover);addEvents(d,currentDate,month,year);r.append(d);currentDate.addDays(1);}tbody.append(r);}calendarTable.append(tbody);$("#ec-calendar").empty().append(calendarTable);}function displayNewMonth(ele,m,y){ele.blur();var t;if((!m&&!y)||(isNaN(m)&&isNaN(y))){t=new Date().zeroTime();t.setDate(1);}else{if(isNaN(m)){t=new Date(displayedYear+y,displayedMonth,1);}else{if(isNaN(y)){t=new Date(displayedYear,displayedMonth+m,1);}else{t=new Date(displayedYear+y,displayedMonth+m,1);}}}displayedMonth=t.getMonth();displayedYear=t.getFullYear();getEvents(displayedMonth,displayedYear,function(){$("h4",$c).html(Date.monthNames[displayedMonth]+" "+displayedYear);renderCalendar(displayedMonth,displayedYear);});return false;}});